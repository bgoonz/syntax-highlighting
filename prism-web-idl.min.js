!(({ languages }) => {
  const n = "(?:\\B-|\\b_|\\b)[A-Za-z][\\w-]*(?![\\w-])";

  const t = `(?:\\b(?:unsigned\\s+)?long\\s+long(?![\\w-])|\\b(?:unrestricted|unsigned)\\s+[a-z]+(?![\\w-])|(?!(?:unrestricted|unsigned)\\b)${n}(?:\\s*<(?:[^<>]|<[^<>]*>)*>)?)(?:\\s*\\?)?`;

  const i = {};
  for (const r in ((languages["web-idl"] = {
  ...
  languages["web-idl"]))
    "class-name" !== r && (i[r] = languages["web-idl"][r]);

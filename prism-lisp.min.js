!(function({util, languages}) {
  ...
  const l = {
    ...
    keyword: [
      {
        pattern: RegExp(
          `${r}(?:and|(?:cl-)?letf|cl-loop|cond|cons|error|if|(?:lexical-)?let\\*?|message|not|null|or|provide|require|setq|unless|use-package|when|while)${i}`
        ),
        lookbehind: !0,
      },
      {
  ...
  (l.lambda.inside.arguments = d),
    (l.defun.inside.arguments = util.clone(d)),
    (l.defun.inside.arguments.inside.sublist = d),
    (languages.lisp = l),
    (languages.elisp = l),
    (languages.emacs = l),
    (languages["emacs-lisp"] = l);
})(Prism);

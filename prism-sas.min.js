!(e => {
  const t = "(?:\"(?:\"\"|[^\"])*\"(?!\")|'(?:''|[^'])*'(?!'))";
  const a = /\b(?:\d[\da-f]*x|\d+(?:\.\d+)?(?:e[+-]?\d+)?)\b/i;
  const n = { pattern: RegExp(t + "[bx]"), alias: "number" };
  const i = { pattern: /&[a-z_]\w*/i };
  ...
  const l = { pattern: RegExp(t), greedy: !0 };
  const c = /[$%@.(){}\[\];,\\]/;
  const d = { pattern: /%?\b\w+(?=\()/, alias: "keyword" };

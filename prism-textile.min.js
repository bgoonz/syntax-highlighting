!(({ languages }) => {
  ...
  const i = {
    css: { pattern: /\{[^{}]+\}/, inside: { rest: languages.css } },
    "class-id": {
      pattern: /(\()[^()]+(?=\))/,
      lookbehind: !0,
      alias: "attr-value",
  ...
  const t = (languages.textile = languages.extend("markup", {
    phrase: {
      pattern: /(^|\r|\n)\S[\s\S]*?(?=$|\r?\n\r?\n|\r\r)/,
      lookbehind: !0,
      inside: {
  ...
  const r = a.inline.inside;
  (r.bold.inside = o),
    (r.italic.inside = o),
    (r.inserted.inside = o),
    (r.deleted.inside = o),
    (r.span.inside = o);
  const d = a.table.inside;
  (d.inline = o.inline),
    (d.link = o.link),
    (d.image = o.image),
